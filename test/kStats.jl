...
@testset "kStats.jl tests" begin 
    @testset "better_stirling correctness" begin
        @test ThorinDistributions._better_stirling(10,5)[10,5]==Combinatorics.stirlings2(10,5)
    end
    
    @testset "_fd correctness" begin
        @test ThorinDistributions._fd(3,0) == Polynomials.Polynomial([0,-6,11,-6,1])
        @test ThorinDistributions._fd(3,1) == Polynomials.Polynomial([-6,11,-6,1])
        @test ThorinDistributions._fd(3,2) == Polynomials.Polynomial([6,-5,1])
    end
    
    @testset "_compute_k correctness" begin
        DynamicPolynomials.@polyvar N
        DynamicPolynomials.@polyvar S[1:9]
        result = ThorinDistributions._compute_k(N,S);
